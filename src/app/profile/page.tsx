...
export default function ProfilePage() {
  ...
  const [form, setForm]           = useState({ nombre: '', email: '', ubicacion: '', avatar: '', rol: 'BUYER' });
  ...
  useEffect(() => {
    ...
    if (stored) {
      setUser(stored);
      setForm({ nombre: stored.nombre || '', email: stored.email || '', ubicacion: stored.ubicacion || '', avatar: stored.avatar || '', rol: stored.rol || 'BUYER' });
    }
  }, []);
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      // El rol solo se manda para comprador/vendedor; un admin no se reasigna acá.
      const payload = user?.rol === 'ADMIN'
        ? { nombre: form.nombre, email: form.email, ubicacion: form.ubicacion, avatar: form.avatar }
        : form;
  ...
  const ROL_LABEL: Record<string, string> = { BUYER: 'Comprador', SELLER: 'Vendedor', ADMIN: 'Administrador' };
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
      <div className="mb-6">
        ...
        <p className="text-slate-500 text-sm mt-1">Gestiona tu información personal y tus productos</p>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm">
        ...
        <div className="p-6 space-y-4">
          ...
          {user.rol !== 'ADMIN' && (
            <div>
              <label className="text-xs font-medium text-slate-500 block mb-1.5">Rol</label>
                ...
                {(['BUYER', 'SELLER'] as const).map((r) => {
                  ...
                  return (
                    <button
                      key={r}
                      type="button"
                      onClick={() => editing && setForm({ ...form, rol: r })}
                      disabled={!editing}
                      className={`px-3 py-2 rounded-lg text-sm font-medium border transition-colors disabled:cursor-not-allowed ${
                        active
                          ? 'bg-indigo-600 border-indigo-600 text-white'
                          : 'bg-white border-slate-200 text-slate-600 hover:border-indigo-300 disabled:hover:border-slate-200 disabled:opacity-60'
                      }`}
                    >
                      {ROL_LABEL[r]}
                    </button>
              ...
              <p className="text-xs text-slate-400 mt-1.5">
                Como vendedor publicás productos; como comprador navegás y comprás. Podés cambiarlo cuando quieras.
              </p>
            </div>
          )}
          <div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
        ...
        <dl className="space-y-2 text-sm">
          {[
            ...
            ...(user.rol === 'ADMIN' ? [] : [
              { dt: 'Productos publicados', dd: String(garments.length) },
              { dt: 'Productos verificados', dd: String(garments.filter(g => g.verificationStatus === 'APPROVED').length), highlight: true },
            ]),
          ].map(({ dt, dd, highlight }) => (
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm">
        <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">Tus productos y sus pasaportes NFT</p>
          </div>
          <Link href="/sell"
            ...
            <Plus className="w-3.5 h-3.5" /> Publicar producto
          </Link>
        </div>
        ...
        <div className="p-6">
          {loadingGarments ? (
            ...
            <div className="text-center py-10">
              ...
              <p className="text-slate-500 text-sm">Todavía no publicaste ningún producto.</p>
